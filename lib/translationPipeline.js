...
function looksLikeReferenceStart(line = '') {
  return /^\s*(?:\[\d+\]|\d+[\.)])\s+/.test(line)
    || /^\s*[A-Z][A-Za-z'.-]+,\s+[A-Z]/.test(line)
    || /^\s*[A-Z]\.\s*[A-Z][A-Za-z'.-]+/.test(line);
}

function cleanReferenceEntry(entry = '') {
  return String(entry)
    .replace(/^\s*(?:\[\d+\]|\d+[\.)])\s*/, '')
    .replace(/\s+/g, ' ')
    .replace(/\s+([,.;:])/g, '$1')
...
function formatReferenceSection(referenceText = '') {
  const normalized = cleanExtractedText(referenceText);
  ...
  for (const line of lines) {
    if (/^(references|bibliography|参考文献|ref erences)$/i.test(line)) continue;

    if (looksLikeReferenceStart(line) && current) {
      entries.push(cleanReferenceEntry(current));
      current = line;
    } else if (!current) {
      current = line;
    } else {
  ...
  for (const entry of entries) {
    ...
    if (/(.{8,40})\1{3,}/.test(entry)) continue;
  ...
  return [
    '参考文献',
    '',
    ...uniqueEntries.map((entry, index) => `${index + 1}. ${entry}`),
  ].join('\n');
}
...
async function translateDocumentText({
  ...
  return [translatedMain, referenceText]
    .map((part, index) => (index === 1 ? formatReferenceSection(part) : part))
    .filter(Boolean)
    .join('\n\n');
}
