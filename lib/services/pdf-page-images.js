...
const MAX_RENDER_PAGES = normalizePageLimit(process.env.PDF_IMAGE_FALLBACK_MAX_PAGES, 60);
...
async function renderPdfPagesToImages(pdfBase64, options = {}) {
  ...
  const pageNumbers = getRenderPageNumbers(pageCount, {
...
function normalizePageLimit(value, fallback = MAX_RENDER_PAGES) {
  if (value === null || value === 'all' || Number(value) === 0) {
    return null;
  }
...
function normalizeExplicitPageNumbers(pageCount, pageNumbers = []) {
  const seen = new Set();
  return pageNumbers
...
function getRenderPageNumbers(pageCount, pageLimitOrOptions = MAX_RENDER_PAGES, allowPartial = false) {
  const options = typeof pageLimitOrOptions === 'object' && pageLimitOrOptions !== null
    ? pageLimitOrOptions
    : { pageLimit: pageLimitOrOptions, allowPartial };

  if (Array.isArray(options.pageNumbers)) {
    return normalizeExplicitPageNumbers(pageCount, options.pageNumbers);
  }

  const pageLimit = normalizePageLimit(options.pageLimit, MAX_RENDER_PAGES);
  if (pageLimit !== null && pageCount > pageLimit && !options.allowPartial) {
    throw new Error(`PDF 共 ${pageCount} 页，已超过图片视觉翻译上限 ${pageLimit} 页`);
  }

  const count = pageLimit === null
    ? pageCount
    : (options.allowPartial ? Math.min(pageCount, pageLimit) : pageCount);
