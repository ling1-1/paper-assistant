...
function sanitizeModel(model, source = 'builtin') {
  ...
  const hasKey = hasSavedKey || hasEnvKey;
  const textConfigured = Boolean(hasKey && model.textModel);
  const visionConfigured = Boolean(hasKey && model.supportsVision && model.visionModel);
