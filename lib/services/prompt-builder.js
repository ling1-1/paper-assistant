...
3. 必须完整处理每一张图片，从页面顶部到页面下半部分逐段检查，不能只翻标题、摘要或上半页。
4. 输出必须是完整译文，不要概括或摘要，不要把正文压缩成提纲。
5. 只输出译文正文，按页面和段落自然衔接；不要输出“第 X 页译文”等说明。
6. 对表格优先保留原始数值、单位、化学式和列关系；无法可靠识别的表格区域请简要标注“表格内容识别不完整”，不要逐字复写乱码。
7. 参考文献、作者列表、期刊名、卷期页码原则上保留原文格式，不要强行翻译人名、刊名或参考文献条目。
8. 忽略页眉、页脚、页码、扫描噪声和重复字符；如果看到类似 MSMSMS、乱码串、重复符号，请直接省略。
9. 不要使用 Markdown 标题符号、代码块、emoji 或额外解释。
10. 不要输出 HTML 标签或富文本标记，例如 <sub>、<sup>、<br>；化学式可使用普通文本或 Unicode 上下标。`;
}
