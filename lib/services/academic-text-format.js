...
function normalizeHtmlMathTags(text = '') {
  return String(text)
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&amp;/g, '&')
    .replace(/<\s*br\s*\/?\s*>/gi, '\n')
    .replace(/<\s*sub\s*>(.*?)<\s*\/\s*sub\s*>/gi, (_, value) => (
      convertMappedChars(String(value).replace(/<[^>]*>/g, ''), SUBSCRIPT_MAP)
    ))
    .replace(/<\s*sup\s*>(.*?)<\s*\/\s*sup\s*>/gi, (_, value) => (
      convertMappedChars(String(value).replace(/<[^>]*>/g, ''), SUPERSCRIPT_MAP)
    ))
    .replace(/<\/?\s*(?:sub|sup)\s*\/?\s*>/gi, '');
}

function normalizeCommonFormulaText(text = '') {
  return String(text)
    .replace(/\\%/g, '%')
    .replace(/\\frac\s*2\s*[×x]\s*生成的\s*H₂?\s*分子数\s*入射光子数/gi, '(2 × 生成的H₂分子数 / 入射光子数)')
    .replace(/\\frac\s*2\s*[×x]\s*生成的\s*H2\s*分子数\s*入射光子数/gi, '(2 × 生成的H₂分子数 / 入射光子数)')
    .replace(/\\frac\{([^{}]+)\}\{([^{}]+)\}/g, '($1 / $2)');
}

function unwrapLatexCommands(text = '') {
  return normalizeCommonFormulaText(normalizeHtmlMathTags(text))
