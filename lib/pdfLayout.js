...
function splitReadingColumns(items, pageWidth) {
  const contentItems = items.filter((item) => item.width > 0 && item.text);
  const left = [];
  const right = [];

  for (const item of contentItems) {
    const center = item.x + item.width / 2;
    if (center < pageWidth * 0.54) {
      left.push(item);
    } else {
      right.push(item);
    }
  ...
  const hasTwoColumns = left.length >= 12 && right.length >= 12;
  if (!hasTwoColumns) {
    return [contentItems];
  }

  return [left, right].filter((column) => column.length);
}
...
export async function extractPdfLayout(buffer) {
  ...
  for (let pageNumber = 1; pageNumber <= doc.numPages; pageNumber += 1) {
    ...
    const columns = splitReadingColumns(items, viewport.width);
    const blocks = columns.flatMap((columnItems, columnIndex) => {
      const lines = clusterLines(columnItems);
      return clusterBlocks(lines).map((block) => ({
        ...block,
        id: `block-${columnIndex + 1}-${block.id.replace('block-', '')}`,
